...
fn main() {
    ...
    let mut reader = {
        if let Some(path) = cli_args.value_of("input") {
            Box::new(BufReader::new(File::open(path).unwrap())) as Box<BufRead>
        } else {
    ...
    while let Ok(num_read) = reader.read(&mut buffer) {
        ...
        if cli_args.is_present("decode") {
            for ch in buffer.take(num_read as u64).chars() {
                let ch = ch.expect("Input contains invalid UTF-8") as u32;
                let write_data = match dec.get(&ch) {
                    Some(ch) => *ch,
                    None => ch as u8
                };
                writer.write(&[write_data]).expect("Write error");
            }
        } else {
            for byte in buffer.iter().take(num_read) {
                let ch: char = char::from_u32(co[*byte as usize]).unwrap();
                writer.write(ch.encode_utf8(&mut write_buf).as_bytes()).expect("Write error");
            }
        }
